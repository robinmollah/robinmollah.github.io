...
#### 100KB Web Server
Built complex web servers running in a 64MB RAM wifi router with custom security layers and TCP tunneling inside tightly constrained Wi‑Fi router environments. ([Bonton Connect](https://bonton.app))

#### 36MB Game Engine
Engineered and optimized a LibGDX framework-based game engine and full open-world game to operate within strict memory and storage limits in a **512MB RAM** Android Device. Reworked asset management, minimized runtime footprint, streamlined execution paths, and restructured architecture for stability under hard caps. ([71: Hit & Run](https://apkpure.com/71-hit-run/net.properbd.hitnrun))

#### Single digit latency
Engineered and optimized a real-time streaming system to operate under controlled environment in <=9m latency. ([Eagle 3D Streaming](https://eagle3dstreaming.com/))
...
#### Ultra large file transfer
Optimized uploading a file from a regular user to distribute it through a complex cloud network facing and solving browser limitations, network constraints, and cloud I/O. >300GB files per upload. Speed increased to ~10x. ([Eagle 3D Streaming](https://eagle3dstreaming.com/))

#### Custom VPN protocol & binary‑level TCP optimization
Designed and implemented a custom VPN communication protocol enabling real-time device-to-device communication applying low-level TCP and binary payload optimizations. ([Bonton Connect](https://bonton.app))

#### Custom web server protocol
Built a custom webserver protocol for real-time communication between devices through several proxies and binary level optimisations on TCP protocol. ([Bonton Connect](https://bonton.app))
